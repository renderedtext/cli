shared_examples "associated_with_shared_config" do
  ...
  let(:shared_config_name) { "shared_config" }
  ...
  describe ".list_for_shared_config" do
    ...
    it "calls info on sem_api_shared_configs" do
      expect(Sem::API::SharedConfigs).to receive(:info).with(org_name, shared_config_name)

      described_class.list_for_shared_config(org_name, shared_config_name)
    end
    ...
    it "returns the instance hashes" do
      return_value = described_class.list_for_shared_config(org_name, shared_config_name)
  ...
  describe ".add_to_shared_config" do
    ...
    it "calls info on the sem_api_shared_configs" do
      ...
      described_class.add_to_shared_config(org_name, shared_config_name, params)
    end
  ...
  describe ".remove_from_shared_config" do
    ...
    it "calls list_for_shared_config on the described class" do
      expect(described_class).to receive(:list_for_shared_config).with(org_name, shared_config_name)

      described_class.remove_from_shared_config(org_name, shared_config_name, instance_name)
    end
